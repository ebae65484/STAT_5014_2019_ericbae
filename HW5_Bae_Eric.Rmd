...
I defined my tolerance level as $\sum_i^n (h_0(x_i) - h_1(x_i))^2 < 0.001$. Doing so resulted in my $\hat{\Theta}_0 \approx 0.9695$ and $\hat{\Theta}_1 \approx 2.0016$.  

My $\alpha$, the step size, was 2.  

These outputs were identical to that of the lm function.  
...
```{r y reduced time, echo = T, eval = T, include = T}
system.time({
  y <- p + A %*% (q - r)
})
```
...
While this may not appear to be too good, it is infinitely better than "Error". So I will take it. 
